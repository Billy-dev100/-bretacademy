function fileClosure() {
  ...
  function loadPayPalWidget(parent) {
    const container = createModal();
    pushClass(container, 'modal_register')
    const body = createEl()
    ...
    // header.textContent = payPalPromptText;
    // header.style.marginBottom = `24px`;
    // button.id = payPalButtonID;
    // container.appendChild(body);
    // appendModal(parent, true)
    // payUp();
  }
  ...
  function submitSignUp(form) {
    ...
    const formCarryID = form.dataset.action;
    const formAction = `https://formcarry.com/s/${formCarryID}`;
    const thisFormID = form.id;
    ...
    .then(() => {
      ...
      if(register == thisFormID) {
        console.log("on register form", formCarryID);
        extraContent = elem('.hidden');
        pushClass(extraContent, 'wrap_min');
        loadPayPalWidget(form);
        let thisModal = elem('.modal');
        thisModal.firstElementChild.appendChild(extraContent);
      } else {
        console.log("on normal contact form", formCarryID);
  ...
  // const iframes = elems('iframe');
  // const googleIframe = iframes.length ? Array.from(iframes).pop() : null;

  // if(googleIframe) {
  //   googleIframe.addEventListener('load', function(event){
  //     setTimeout(function(e) {
  //       console.log(googleIframe, event);
  //     }, 1000);
  //   });
  // }
